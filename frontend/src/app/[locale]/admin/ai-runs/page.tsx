          ...
          <table className="w-full min-w-[900px]">
            <thead>
              <tr className="border-b" style={{ borderColor: "var(--color-border)", background: "var(--color-surface-alt)" }}>
                ...
                <th className="px-6 py-4 text-center text-sm font-bold" style={{ color: "var(--color-text-secondary)" }}>{t("admin.modelCol")}</th>
                <th className="px-6 py-4 text-center text-sm font-bold" style={{ color: "var(--color-text-secondary)" }}>{t("admin.tokensCol")}</th>
                <th className="px-6 py-4 text-center text-sm font-bold" style={{ color: "var(--color-text-secondary)" }}>{t("admin.costCol")}</th>
                <th className="px-6 py-4 text-center text-sm font-bold" style={{ color: "var(--color-text-secondary)" }}>{t("admin.durationCol")}</th>
                <th className="px-6 py-4 text-center text-sm font-bold" style={{ color: "var(--color-text-secondary)" }}>{t("admin.createdCol")}</th>
              ...
              {runs.map((run) => (
                <Fragment key={run.id}>
                  <tr className="border-b cursor-pointer transition-colors hover:opacity-90" style={{ borderColor: "var(--color-border)" }} onClick={() => setExpanded(expanded === run.id ? null : run.id)}>
                    ...
                    <td className="px-6 py-4 text-center text-xs" style={{ color: "var(--color-text-muted)" }}>{run.model_used}</td>
                    <td className="px-6 py-4 text-center text-sm font-bold" style={{ color: "var(--color-text)" }}>{fmtTokens(run.tokens_used)}</td>
                    <td className="px-6 py-4 text-center text-xs" style={{ color: "var(--color-text-muted)" }}>{fmtCost(run.cost)}</td>
                    <td className="px-6 py-4 text-center text-xs" style={{ color: "var(--color-text-muted)" }}>{fmtMs(run.duration_ms)}</td>
